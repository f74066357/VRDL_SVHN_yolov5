...
def test(
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    with open(
                        save_dir / "labels" / (path.stem + ".txt"), "a"
                    ) as f:
            ...
            if save_json:
                ...
                image_id = int(
                    path.stem
                )if path.stem.isnumeric() else path.stem
    ...
    if not training:
        print(
            "Speed: %.1f/%.1f/%.1f ms inference/NMS/total per %gx%g \
            image at batch-size %g"
            % t
        )
    ...
    if save_json and len(jdict):
        ...
        try:
            ...
            # blob/master/PythonAPI/pycocoEvalDemo.ipynb
    ...
    if not training:
        s = (
            ...
            labels saved to {save_dir / 'labels'}"
            if save_txt
            else ""
        )
