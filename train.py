...
try:
    ...
    wandb = None
    logger.info(
        "Install Weights & Biases for experiment \
        logging via 'pip install wandb' (recommended)"
    )
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if pretrained:
        ...
        if hyp.get("anchors"):
            ...
            # force autoanchor
        model = Model(
            opt.cfg or ckpt["model"].yaml, ch=3, nc=nc
        ).to(device)  # create
    ...
    if wandb and wandb.run is None:
        ...
        wandb_run = wandb.init(
            config=opt,
            resume="allow",
            project="YOLOv5" if opt.project == "runs/train"
    ...
    if pretrained:
        ...
        if epochs < start_epoch:
            logger.info(
                "%s has been trained for %g epochs.\
...
if __name__ == "__main__":
    ...
    with open(opt.hyp) as f:
        ...
        if "box" not in hyp:
            warn(
                'Compatibility: %s missing "box" \
                which was renamed from "giou" in %s'
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            logger.info(
                f'Start Tensorboard with "tensorboard \
    ...
    # Evolve hyperparameters (optional)
        ...
        print(
            f"Hyperparameter evolution complete.\
            Best results saved as: {yaml_file}\n"
